...
@Slf4j
@Service
public class ValidateJitGuard implements Guard<OrderStatus, OrderUpdateEvent> {

    @Override
    public boolean evaluate(StateContext<OrderStatus, OrderUpdateEvent> context) {
        log.info("Executing ValidateJitGuard for order {}");
        OrderEntry orderEntry = (OrderEntry) context.getMessageHeader(StateMachineConstants.ORDER);
        log.info("Validating inside guard: ValidateOnHandGuard {}", orderEntry.getId());
        if (OrderSupplierType.JIT.equals(orderEntry.getOrderSupplierType())) {
            return true;
        } else {
            throw new GuardHackException("Order of wrong type {}", orderEntry.getId());
        }
    }
}
